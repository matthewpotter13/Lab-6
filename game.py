...
def execute_take(item_id):
    ...
    for item in current_room['items']:
        if item["id"] == item_id and item["mass"] + max_mass <= 5:
            max_mass = max_mass + item["mass"]
            print ("you took " + item['name'] )
            inventory.append(item)
            current_room['items'].remove(item)
            return

    print ("Cant do that.\nTry dropping something first or picking up a different item.")
...
def main():
    ...
    # Main game loop
    while True:
        if current_room == rooms['Admins'] and current_room['items']==[item_laptop]:
            print ("\nYou win the game!")
            break
